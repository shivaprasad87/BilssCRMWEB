    ...
    function site_visit_report_mail(){
       ...
       /* if ($this->session->userdata('user_type') == 'user') {
        ...
        public function send_project_data_email($value='')
        {
            ...
            $message = "Hi,
<br><br>
            Greetings From Blisss Realty! Please find the ".$c[0]->name." project details attached below.
<br><br><br>
<br>
<br>
Regards, <br>
".$this->session->userdata("user_name").",<br>".$this->session->userdata("user_email").".";
    ...
    public function Gcal($value='')
    {
       $this->import_classes();
       $data = Gcalendar('shivas8787@gmail.com');
       print_r($data);
    }
        public function leadBucket() {
        $data['name'] ="leadBucket";
        $data['heading'] ="leadBucket";
        $where="";
        $rowCount               = $this->common_model->count_onlineleads('online_leads','99acres');
        $data["totalRecords"]   = $rowCount;

        $data["links"]          = paginitaion(base_url().'dashboard/leadBucket/', 3,20, $rowCount);
        $page = $this->uri->segment(3);
        $offset = !$page ? 0 : $page;
        //------ End --------------

        $data['leads'] = $this->common_model->get_online_leads('online_leads','',20,$offset);
        $user_type = $this->session->userdata("user_type");
        $this->load->view('leadBucket',$data);

    }
        public function commitLead($id='',$broker='',$pid=''){
        $error=0; 
            $dept=1;
            $callback_type=2;
            ...
            $due_date=date('Y-m-d');
            $due_time=date('h:m'); 
            $lead_data = $this->common_model->getFromId($id, 'id', 'online_leads');
            $p_id['id']='';
            if($p_id)
                $p_id['id']=$pid;
            else
            ...
            if($broker=='99acres')
                ...
                $data=$this->common_model->getsourceId(urldecode($broker));
                //echo $this->db->last_query();die();
                 $lead_ids = json_decode(json_encode($this->callback_model->get_last_id()),true);
            $leadId = $lead_ids['id']+1;
                if(trim($lead_data->name)=='')
                {
                     echo "<script>alert('THis lead taken by another user');location.href='".base_url('dashboard/leadBucket/')."'</script>";
                     die;
                }
                $data=array(
                    'dept_id'=>1,
                ...
                if (!($this->callback_model->isexists_callbacks($data)))
                {
                    $this->callback_model->add_callbacks($data);
                     $this->common_model->deleteWhere(array('id'=>$id), 'online_leads');
                     echo "<script>alert('added successfully');location.href='".base_url('dashboard/leadBucket/')."'</script>";
                ...
                {
                    $error=1;
                     echo "<script>alert('this lead already exists');location.href='".base_url('dashboard/leadBucket/')."'</script>";
                    //echo "<script>location.href='".current_url()."'</script>";
