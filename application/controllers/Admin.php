	...
	public function generate_callback() {
		if($this->input->post()){
			...
			if($this->input->post('ref_by'))
			{
				...
				'leadid'=> trim("BLS-".sprintf("%'.011d",$lead_ids).PHP_EOL),
				'user_id'=>$user_name,
				'due_date'=>$due_date,
...
{		
		...
		if($this->input->post('sitevisit_date') && $this->input->post('sitevisit')){
			...
			require_once  FCPATH. "quickstart.php";
			 ...
			 $data = array($id,$this->session->userdata('email'),'Site Visit Fixed',$this->input->post('sitevisit_date').'T10:00:00+05:30',$location,$this->input->post('sitevisit_date').'T10:00:00+05:30'); 
       		 Gcalendar($data);
		}
	...
	function bulk_upload_callback() {
		if(isset($_POST["submit"])) {
			...
			if (move_uploaded_file($_FILES["upload_xl"]["tmp_name"], $target)){
				...
				//$leadIdKey = "BLS - ".sprintf("%'.011d",$lead_ids).PHP_EOL;
...
public function make_user_online($value='')
	...
	$data['name'] ="admin";
		...
		if($value)
		{
			...
			$to_emails ="khankamal5555@gmail.com"; 
			$this->load->library('email');
			$config = email_config();
			
			$this->email->initialize($config);
			$this->email->from("khankamal5555@gmail.com","Admin");
			$this->email->to($to_emails);
			$this->email->subject($subject);
			$this->email->message($mail_body);
			//$this->email->send();
...
//echo $this->db->last_query();die;
    ...
    public function delete_files($table_of_image = 'property_data_files')
    {
        ...
        $this->common_model->deleteWhere(array('file_name' => $path), $table_of_image);
